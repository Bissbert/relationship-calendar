...
- **Rose** (`.btn-rose`): Rose fill, ink text — the primary call-to-action wherever the surrounding ground is velvet ("Copy share link," "Download calendar file," banner's "Add them"). On `pointer: coarse` devices "Add to my calendar" takes the Rose slot in the calendar card and the download steps down to a ghost button, "Or download the file."

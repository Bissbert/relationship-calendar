...
The page is built from a single `.wrap` container (`width: min(100% - 2rem, 70rem)`, centered): a 1rem gutter on narrow viewports, capping at 1120px. Three regions use the same two-column grammar at the `60rem` (960px) breakpoint — a flexible primary column beside a fixed `26rem` (416px) secondary column: the header (names/counter beside the "Next up" ticket), and the board (the dates list beside the composer card, which becomes `position: sticky; top: 1.25rem`). Below `60rem` the composer reorders above the list (`order: -1`) because adding a date is the primary mobile action, per the FIRST VIEWPORT contract — it is not simply stacked in source order. To keep the dates close to the fold, the composer there starts folded to its heading band with a round Stamp Ink plus button (the whole band is the tap target); it opens on tap, on Edit, on "Add a date" from an empty state or a day, and starts open when the calendar is empty. Below `30rem` ticket actions stay on one row ("Google Calendar" shortens to "Google", the accessible name stays full), and the two-up form rows stack; below `40rem` the filter chips become one horizontally scrolling row that bleeds to the screen edge.
...
- **Rose** (`.btn-rose`): Rose fill, ink text — the primary call-to-action wherever the surrounding ground is velvet ("Copy share link," "Download calendar file"). Next to the download, the ghost button "How do I add it?" opens the calendar help dialog.

...
import { shareLink, readShareLink, toBackup, fromBackup, googleCalendarLink, download, calendarHelpPlatform } from './share.js';
...
function renderTakeaway() {
  ...
  const label = n ? `Download calendar file · ${plural(n, 'date')}` : 'Download calendar file';
  $('export-label').textContent = label;
  $('cal-help-download-label').textContent = label;
  for (const id of ['export-btn', 'cal-help-download', 'share-btn', 'backup-btn', 'clear-btn']) $(id).disabled = !n;
...
$('export-btn').addEventListener('click', downloadCalendar);
$('cal-help-download').addEventListener('click', downloadCalendar);

// No browser can write to the phone's calendar, so the help dialog leads with
// the steps for the device in hand and keeps the others one tap away.
const helpPlatform = calendarHelpPlatform({ userAgent: navigator.userAgent, maxTouchPoints: navigator.maxTouchPoints });
$('cal-help-here').append(document.querySelector(`.cal-help-steps[data-platform="${helpPlatform}"]`));

$('cal-help-btn').addEventListener('click', () => {
  const dialog = $('cal-help');
  dialog.querySelector('.cal-help-other').open = false;
  dialog.showModal();
  (state.events.length ? $('cal-help-download') : dialog.querySelector('button[type="submit"]')).focus();
});
